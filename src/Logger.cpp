...
Logger::Logger(const std::string &configFileName)
    : logger(initLogger(configFileName)) {}

std::shared_ptr<spdlog::logger> Logger::getLogger() const { return logger; }

std::shared_ptr<spdlog::logger>
Logger::initLogger(const std::string &configFileName) {
  ...
  if (jsonConfig.contains("log_level") && jsonConfig["log_level"].is_string()) {
    ...
    auto logger = std::make_shared<spdlog::logger>(
        "main_logger", spdlog::sinks_init_list{file_sink});
